...
# Function to Get Total Members from Database
def get_member_count():
    try:
        conn = sqlite3.connect("gym_users.db")  # Database Connection
        cursor = conn.cursor()
        cursor.execute("SELECT COUNT(*) FROM gym_users")  # Fetch Total Members
        count = cursor.fetchone()[0]  # Get Count Value
        conn.close()
        return count
...
# Function to Update Member Count in UI
def update_members():
    global member_count
    new_count = get_member_count()  # Get Latest Count
    if new_count != member_count:  # If Count Changed, Update Label
        member_count = new_count
        members_label.config(text=f"Total Members: {member_count}")  

    root.after(5000, update_members)  # Call Again After 5 Seconds
...
# Label to display member count
members_label = tk.Label(content, text=f"Total Members: {member_count}", font=("Arial", 12), bg="gray", fg="white", width=20, height=5)
members_label.pack(pady=20)

# Initialize database
create_db()

# Start Auto-Update for Member Count
update_members()
